...
from drive import get_worksheet, next_available_row, update_cell
...
if "-m" in sys.argv:
    ...
    if "-r" in sys.argv:
        ...
        if row_number and row_number >= 2:
            try:
                ...
                update_cell(ws, row_number, 3, message)
                check = 1
        ...
        display_loading_message("Updating message", "Message updated")
        update_cell(ws, previous_row, 3, message)

    try:
        if "-d" in sys.argv:
            display_loading_message("Updating date", "Date updated")
            if check == 1:
                update_cell(ws, row_number, 2, get_date())
                pass
            update_cell(ws, previous_row, 2, get_date())
        hide_loading_message_with_error(False)
    except:
        hide_loading_message_with_error(True)
    ...
    try:
        if "-t" in sys.argv:
            ...
            update_cell(ws,previous_row, 4, description)
        hide_loading_message_with_error(False)
    except:
        hide_loading_message_with_error(True)
